...
src="figures/6_kg_example.png">
<br>
<div style="color:orange; border-bottom: 1px solid #d9d9d9;
display: inline-block;
color: #999;
padding: 2px;">图6-1. 一个生物医学异质图</div>
</center>
...
<img style="border-radius: 0.3125em;
box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);"
src="figures/6_Knowledge_graph_example_grape.png">
<br>
<div style="color:orange; border-bottom: 1px solid #d9d9d9;
display: inline-block;
color: #999;
padding: 2px;">图6-2. 一个知识图谱片段</div>
</center>
...
$$
h_i^{(l+1)}=\sigma (\sum_{j \in \mathcal{N}_i} \frac{1}{c_i}W^{(l)}h_j^{(l))} + W_{0}^{(l)}h_{i}^{(l)}) 
$$
...
$$
h_{i}^{(l+1)}=\sigma(\sum_{r \in \mathcal{R}} \sum_{j \in \mathcal{N}_{i}^{r}} \frac{1}{c_{i,r}}W_{r}^{(l)}h_{j}^{(l)} + W_{0}^{(l)}h_{i}^{(l)})
$$

其中 $\mathcal{N}_i^r$ 表示关系 $r$ 下节点 $i$ 的邻居索引集，其中 $c_{i,r}$ 是针对不同问题下的一个归一化常量，可以通过学习得到或预先设定（如 $c_{i,r}=|\mathcal{N}_i^r|$ 表示边类型为 $r$ 的邻居数量）。对比之前的 GCN 公式，我们可以看出主要变化是将 $W^{(l)}$ 变成了与关系类型相关的 $W_r^{(l)}$，以及加和所有关系。当所有关系类型一致时，R-GCN 退化为 GCN。
...
### 6.3.1 基底分解
假设第 $l$ 层关系 $r$ 下的权重 $W_{r}^{(l)}$ 可以被分解为基底 $\{V^{l}_1,V^{l}_2,\cdots,V^{l}_b,\cdots , V^{l}_B\}$ 的线性组合，即
$$
W_{r}^{(l)}=\sum_{b=1}^{B} c_{r b}^{(l)} V_{b}^{(l)}
$$

其中 $V^{(l)}_b \in \mathbb{R}^{d^{(l+1)}×d^{(l)}}$ 是基底矩阵，$B$ 表示基底的数量，$c^{(l)}_{rb}$ 表示基底 $V^{(l)}_b$ 对于关系 $r$ 的重要程度。通过以上基底分解，我们可以将原本需要学习 $\{W_{r}^{(l)}\}_{r=1}^R$ 的任务简化为学习：
...
<img style="border-radius: 0.3125em;
box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);"
src="figures/6_block_diagonal_decomposition.png">
<br>
<div style="color:orange; border-bottom: 1px solid #d9d9d9;
display: inline-block;
color: #999;
padding: 2px;">图6-6. 块对角矩阵分解示意图</div>
</center>
