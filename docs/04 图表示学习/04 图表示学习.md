...
其中的后验概率 $P(v|\mathbf{z_u})$ 可以被表示为 $P(v|\mathbf{z_u}) = \frac{exp(\mathbf{z_u}^\intercal \mathbf{z_v})}{exp(\sum_{n \in V}\mathbf{z_u}^\intercal \mathbf{z_n})}$。因为我们可以得到优化目标为

$$
\mathcal{L} =\sum_{u\in V} \sum_{v\in N_R (u)} - \ log (\rm \frac{exp(\mathbf{z_u}^\intercal \mathbf{z_v})}{exp(\sum_{n \in V}\mathbf{z_u}^\intercal \mathbf{z_n})})
$$
