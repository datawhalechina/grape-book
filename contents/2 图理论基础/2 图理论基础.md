...
- 其中 $\mathcal{V}=\left\{v_{1}, \ldots, v_{N}\right\}$ 是数量为 $N=|\mathcal{V}|$ 的**节点（node或vertex）**的集合， $\mathcal{E}=\left\{e_{1}, \ldots, e_{M}\right\}$ 是数量为 $M$ 的**边（edge或link）**的集合。
...
- $\mathbf{A}_{i, j}=1$ 表示存在从节点 $v_i$ 到 $v_j$ 的边，$\mathbf{A}_{i, j}=0$ 表示不存在从节点 $v_i$ 到 $v_j$ 的边。
