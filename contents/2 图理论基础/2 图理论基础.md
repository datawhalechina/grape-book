...
- 最短路径被定义为两个点之间的**距离（distance）**。**最短路径（shortest path）** $v_{s}, v_{t} \in \mathcal{V}$ 是图 $\mathcal{G}=\{\mathcal{V}, \mathcal{E}\}$ 上的一对节点，节点对 $v_{s}, v_{t} \in \mathcal{V}$ 之间所有路径的集合记为 $\mathcal{P}_{st}$。节点对 $v_{s}, v_{t}$ 之间的最短路径 $p_{st}^{sp}$ 为 $\mathcal{P}_{st}$ 中长度最短的一条路径，其形式化定义为
$$p_{st}^{sp}=\arg \min_{p \in \mathcal{P}_{st}}|p|$$
其中， $p$表示  $\mathcal{P}_{st}$ 中的一条路径，$|p|$是路径$p$的长度。
...
- 给定一个图 $\mathcal{G}=\{\mathcal{V}, \mathcal{E}\}$，其对应的**关联矩阵**被记为 $\mathbf{M} \in\{0,1\}^{N \times M}$。（抱歉这里我们用加粗的 $\mathbf{M}$ 表示关联矩阵，用不加粗的 $M$ 表示边的个数）
...
- **拉普拉斯矩阵（Laplacian Matrix）**：（也叫做 admittance matrix, Kirchhoff matrix）给定一个图 $\mathcal{G}=\{\mathcal{V}, \mathcal{E}\}$，其邻接矩阵为 $A$，其拉普拉斯矩阵 $L$ 定义为
$$\mathbf{L=D-A}$$
...
- **对称归一化的拉普拉斯矩阵，Symmetric normalized Laplacian）**：给定一个图$\mathcal{G}=\{\mathcal{V}, \mathcal{E}\}$，其邻接矩阵为$A$，其规范化（归一化）的拉普拉斯矩阵定义为
...
<center>
    <img style="border-radius: 0.3125em;
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src="figures/2_network_topology_regular.PNG"> 
    <br>
    <div style="color:orange;
    display: inline-block;
    color: #999;
...
<center>
    <img style="border-radius: 0.3125em;
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src="figures/2_network_topology_other.png"> 
    <br>
    <div style="color:orange;
    display: inline-block;
    color: #999;
...
<center>
    <img style="border-radius: 0.3125em;
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src="figures/2_heterogeneous.png" width=400> 
    <br>
    <div style="color:orange;
    display: inline-block;
    color: #999;
